...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!currentUser && window.location.search.indexOf('skip_auth') === -1) {
    // Don't redirect if already on the main dashboard
    // Just show a toast asking to login
    setTimeout(() => {
      const toast = document.getElementById('toast-container');
      if (toast) {
        showToast('يرجى تسجيل الدخول أولاً من الصفحة الرئيسية', 'warning');
      }
    }, 500);
  }
  
  // Anonymous sign-in for Firebase security
...
async function processBulkFiles(files) {
  ...
  if (!hasPermission('upload_files')) { showToast('غير مصرح لك برفع الملفات', 'error'); return; }
...
function saveFileToStudent(file, attachmentId, student) {
  return new Promise(resolve => {
    ...
    reader.onload = async () => {
      ...
      logActivity('upload_attachment', 'attachment', student.id, student.name, 'رفع ملف: ' + getAttachmentName(attachmentId) + ' للطالب ' + student.name);
      resolve();
    };
...
window.uploadAttachmentFile = async function(input, attachmentId, attachmentName, isCustom) {
  ...
  if (selectedStudent) logActivity('upload_attachment', 'attachment', selectedStudent.id, selectedStudent.name, 'رفع ملف: ' + attachmentName + ' للطالب ' + selectedStudent.name);
};
...
window.deleteAttachment = async function(id, isCustom) {
  ...
  const attName = getAttachmentName(id);
  ...
  logActivity('delete_attachment', 'attachment', selectedStudent.id, selectedStudent.name, 'حذف ملف: ' + attName + ' للطالب ' + selectedStudent.name);
};
...
window.saveCustomAttachment = async function() {
  ...
  try {
    ...
    if (selectedStudent) logActivity('upload_attachment', 'attachment', selectedStudent.id, selectedStudent.name, 'رفع مرفق مخصص: ' + name + ' للطالب ' + selectedStudent.name);
  } catch (e) {
...
window.triggerEditStudent = function() {
  if (!selectedStudent) return;
  if (!hasPermission('edit')) { showToast('غير مصرح لك بالتعديل', 'error'); return; }
  const m = document.getElementById("edit-student-modal"); if (!m) return;
...
window.saveStudentEdits = async function() {
  ...
  if (isOfflineMode || !db || selectedStudent.id.startsWith("mock_")) {
    ...
    Object.assign(selectedStudent, ud); showToast("تم التحديث محلياً.", "success"); closeEditModal(); refreshSelectedStudentDetails(); renderAdminStudentsTable(); logActivity('update_student', 'student', selectedStudent.id, name, 'تعديل بيانات الطالب: ' + name + ' (' + code + ')'); return;
  }
  try { await db.ref(`students/${selectedStudent.id}`).update(ud); Object.assign(selectedStudent, ud); const idx = studentsIndex.findIndex(s => s.id === selectedStudent.id); if (idx !== -1) { studentsIndex[idx] = Object.assign({}, studentsIndex[idx], ud); localStorage.setItem("lotus_students", JSON.stringify(studentsIndex)); } showToast("تم التحديث.", "success"); closeEditModal(); refreshSelectedStudentDetails(); loadSearchIndex().catch(() => {}); logActivity('update_student', 'student', selectedStudent.id, name, 'تعديل بيانات الطالب: ' + name + ' (' + code + ')'); } catch (e) { showToast("خطأ: " + e.message, "error"); }
};
...
window.deleteStudentCompletely = async function() {
  ...
  if (!hasPermission('delete')) { showToast('غير مصرح لك بالحذف', 'error'); return; }
  logToArchive(selectedStudent);
  const delName = selectedStudent.name, delCode = selectedStudent.code, delId = selectedStudent.id;
  if (isOfflineMode || !db || selectedStudent.id.startsWith("mock_")) {
    ...
    showToast("تم الحذف.", "success"); selectedStudent = null; if (studentCard) studentCard.style.display = "none"; if (searchInput) searchInput.value = ""; renderAdminStudentsTable(); renderReports(); logActivity('delete_student', 'student', delId, delName, 'حذف الطالب: ' + delName + ' (' + delCode + ')'); return;
  }
  try {
    await db.ref(`students/${selectedStudent.id}`).remove();
    studentsIndex = studentsIndex.filter(s => s.id !== selectedStudent.id); allStudentsList = allStudentsList.filter(s => s.id !== selectedStudent.id); localStorage.setItem("lotus_students", JSON.stringify(studentsIndex));
    showToast("تم الحذف.", "success"); selectedStudent = null; if (studentCard) studentCard.style.display = "none"; if (searchInput) searchInput.value = ""; loadSearchIndex().catch(() => {}); logActivity('delete_student', 'student', delId, delName, 'حذف الطالب: ' + delName + ' (' + delCode + ')');
  } catch (e) { showToast("خطأ: " + e.message, "error"); }
};
...
window.saveGlobalSettings = function() {
  if (!hasPermission('manage_settings')) { showToast('غير مصرح لك بهذا الإجراء', 'error'); return; }
...
window.adminDeleteStudent = async function(id) {
  ...
  if (!hasPermission('delete')) { showToast('غير مصرح لك بالحذف', 'error'); return; }
  logToArchive(s);
  const delName = s.name, delCode = s.code;
  if (isOfflineMode || !db || id.startsWith("mock_")) {
    ...
    renderRegisteredStudentsTable();
    renderReports();
    logActivity('delete_student', 'student', id, delName, 'حذف الطالب: ' + delName + ' (' + delCode + ')');
    return;
  }
  try {
...
document.addEventListener("DOMContentLoaded", () => {
  const f = document.getElementById("add-student-form"); if (f) f.addEventListener("submit", async e => {
    e.preventDefault(); 
    if (!hasPermission('create')) { showToast('غير مصرح لك بإضافة طلاب', 'error'); return; }
    const name = document.getElementById("add-name-new").value.trim(), code = document.getElementById("add-code-new").value.trim();
    ...
    if (isOfflineMode || !db) { const ns = { id: "student_" + Date.now(), name, code, nationalId, college, level, fileNo, cabinetNo, whatsapp, attachments: {}, delivery: {}, customFields: {}, createdAt: new Date().toISOString() }; studentsIndex.push(ns); allStudentsList.push(ns); localStorage.setItem("lotus_students", JSON.stringify(studentsIndex)); showToast("تم التسجيل محلياً.", "success"); e.target.reset(); renderAdminStudentsTable(); logActivity('create_student', 'student', ns.id, name, 'إضافة طالب: ' + name + ' (' + code + ')'); return; }
    ...
    try { const dr = await db.ref("students").push(ns); ns.id = dr.key; studentsIndex.push(ns); allStudentsList.push(ns); localStorage.setItem("lotus_students", JSON.stringify(studentsIndex)); showToast("تم التسجيل.", "success"); e.target.reset(); renderAdminStudentsTable(); logActivity('create_student', 'student', ns.id, name, 'إضافة طالب: ' + name + ' (' + code + ')'); } catch (err) { showToast("خطأ: " + err.message, "error"); }
  });
});
...
async function importStudentsInBatches(students) {
  ...
  if (ok > 0) logActivity('bulk_import', 'student', '', '', 'استيراد ' + ok + ' طالب من إكسيل');
}
...
window.addCustomDeliveryDoc = async function(studentId) {
  ...
  if (!hasPermission('manage_delivery')) { showToast('غير مصرح لك بإدارة التسليم', 'error'); return; }
  if (!st) return;
...
window.onDeliveryChange = function(studentId, attId, checked) {
  ...
  const attName = getAttachmentName(attId);
  logActivity('delivery_change', 'delivery', studentId, st.name, (checked ? 'تسليم' : 'إلغاء تسليم') + ' مستند: ' + attName + ' للطالب ' + st.name);
};
...
const SESSION_KEY = 'lotus_session';
const ROLE_NAMES = { admin: 'مدير', supervisor: 'مشرف', employee: 'موظف', viewer: 'مشاهد' };
...
function getSession() {
  try {
    ...
    const elapsed = Date.now() - (s.loginTime || 0);
    if (elapsed > 28800000) { clearSession(); return null; }
    return s;
  } catch (e) { return null; }
}
...
function updateUIBasedOnAuth() {
  ...
  if (currentUser) {
    ...
    if (urole) urole.textContent = ROLE_NAMES[currentUser.role] || currentUser.role;
    document.querySelectorAll('.tab-btn[data-screen="admin"]').forEach(el => {
      el.style.display = (currentUser.role === 'admin' || currentUser.role === 'supervisor') ? '' : 'none';
    });

  } else {
    if (menu) menu.style.display = 'none';
    if (prompt) prompt.style.display = 'block';
    document.querySelectorAll('.tab-btn[data-screen="admin"]').forEach(el => { el.style.display = ''; });
  }
}
...
window.AUTH_ON_LOGOUT_CALLBACK = function() {
  ...
  switchScreen('search');
  document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
  const sb = document.querySelector("[data-screen='search']");
  if (sb) sb.classList.add('active');
};

async function checkHasAnyEmployee() {
  if (!isOfflineMode && db) {
...
function saveLocalEmployees(emps) {
...
async function getAllEmployees() {
  ...
  const local = getLocalEmployees();
  local.forEach(e => {
    if (!list.find(x => x.username === e.username)) list.push(e);
  });
  return list;
}
...
async function saveEmployeeToDb(emp) {
  ...
  if (idx >= 0) local[idx] = { ...local[idx], ...emp };
  else local.push(emp);
...
window.renderEmployees = async function() {
  ...
  const count = document.getElementById('employees-count');
  if (!tb) return;
  try {
    const list = await getAllEmployees();
    if (count) count.textContent = '(' + list.length + ')';
    if (list.length === 0) {
      tb.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);">لا يوجد موظفين بعد. أضف أول موظف.</td></tr>';
      return;
    }
    let html = '';
    list.forEach(e => {
      const dateStr = e.lastLogin ? new Date(e.lastLogin).toLocaleString('ar-EG') : '-';
      html += '<tr>' +
        '<td>' + escapeHtml(e.name) + '</td>' +
        '<td>' + escapeHtml(e.username) + '</td>' +
        '<td><span class="role-badge ' + (e.role || 'employee') + '">' + escapeHtml(ROLE_NAMES[e.role] || e.role) + '</span></td>' +
        '<td><span class="status-badge ' + (e.active !== false ? 'active' : 'inactive') + '">' + (e.active !== false ? 'نشط' : 'موقوف') + '</span></td>' +
        '<td style="font-size:0.75rem;color:var(--text-muted);">' + dateStr + '</td>' +
        '<td style="display:flex;gap:5px;">' +
          '<button class="btn-icon" style="padding:4px 8px;display:inline-flex;" onclick="openEditEmployee(\'' + escapeHtml(e.username) + '\')" title="تعديل"><i class="fas fa-pen"></i></button>' +
          '<button class="btn-icon ' + (e.active !== false ? 'btn-delete' : '') + '" style="padding:4px 8px;display:inline-flex;" onclick="toggleEmployeeStatus(\'' + escapeHtml(e.username) + '\')" title="' + (e.active !== false ? 'إيقاف' : 'تفعيل') + '">' +
            '<i class="fas ' + (e.active !== false ? 'fa-pause' : 'fa-play') + '"></i></button>' +
          (currentUser && currentUser.username !== e.username ? '<button class="btn-icon btn-delete" style="padding:4px 8px;display:inline-flex;" onclick="deleteEmployee(\'' + escapeHtml(e.username) + '\')" title="حذف"><i class="fas fa-trash-alt"></i></button>' : '') +
        '</td></tr>';
    });
    tb.innerHTML = html;
  } catch (e) { console.error('renderEmployees error:', e); }
};
...
window.openAddEmployeeModal = function() {
  ...
  document.getElementById('emp-role').value = 'supervisor';
  document.getElementById('employee-modal').dataset.mode = 'add';
  document.getElementById('employee-modal').dataset.editUsername = '';
  document.getElementById('employee-modal-error').style.display = 'none';
  document.getElementById('employee-modal').classList.add('active');
};

window.openEditEmployee = async function(username) {
  const list = await getAllEmployees();
  const emp = list.find(e => e.username === username);
  if (!emp) return;
  ...
  document.getElementById('emp-password').value = '';
  document.getElementById('emp-password').placeholder = 'اتركه فارغاً إذا لا تريد التغيير';
  document.getElementById('emp-role').value = emp.role || 'employee';
  document.getElementById('employee-modal').dataset.mode = 'edit';
  document.getElementById('employee-modal').dataset.editUsername = username;
  document.getElementById('employee-modal-error').style.display = 'none';
  document.getElementById('employee-modal').classList.add('active');
};
...
window.saveEmployee = async function() {
  ...
  const mode = document.getElementById('employee-modal').dataset.mode || 'add';
  const editUsername = document.getElementById('employee-modal').dataset.editUsername || '';
  const errEl = document.getElementById('employee-modal-error');
  if (!name || !username) {
    errEl.textContent = 'الاسم واسم المستخدم مطلوبان';
    errEl.style.display = 'block';
    return;
  }
  if (mode === 'add' && (!password || password.length < 3)) {
    errEl.textContent = 'كلمة المرور يجب أن تكون 3 أحرف على الأقل';
    errEl.style.display = 'block';
    return;
  }
  ...
  try {
    ...
    if (mode === 'edit') {
      ...
      if (list.find(e => e.username === username)) {
        errEl.textContent = 'اسم المستخدم موجود بالفعل';
        errEl.style.display = 'block';
        return;
      }
...
window.toggleEmployeeStatus = async function(username) {
  if (!currentUser || currentUser.role !== 'admin') { showToast('غير مصرح', 'error'); return; }
  if (currentUser.username === username) { showToast('لا يمكن إيقاف حسابك', 'warning'); return; }
  const list = await getAllEmployees();
  const emp = list.find(e => e.username === username);
  if (!emp) return;
...
function getLocalActivityLog() {
  try { return JSON.parse(localStorage.getItem('lotus_activity_log') || '[]'); } catch { return []; }
}

function saveLocalActivityLog(log) {
  if (log.length > 1000) log = log.slice(0, 1000);
  localStorage.setItem('lotus_activity_log', JSON.stringify(log));
}
...
async function getActivityLog(filter) {
  const all = [];
  if (!isOfflineMode && db) {
    try {
      const snap = await db.ref('activityLog').orderByChild('timestamp').limitToLast(500).once('value');
      if (snap.exists()) {
        const data = snap.val();
        Object.values(data).forEach(v => { all.push(v); });
      }
    } catch (e) {}
  }
  const local = getLocalActivityLog();
  local.forEach(e => { if (!all.find(x => x.timestamp === e.timestamp && x.action === e.action && x.employeeUsername === e.employeeUsername)) all.push(e); });
  all.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  if (filter && filter !== '__all__') return all.filter(e => e.action === filter);
...
window.renderActivityLog = async function() {
  ...
  if (!tb) return;
  try {
    const f = filter ? filter.value : '__all__';
    const log = await getActivityLog(f);
    if (count) count.textContent = '(' + log.length + ')';
    if (log.length === 0) {
      tb.innerHTML = '<tr><td colspan="4" style="text-align:center;color:var(--text-muted);">لا يوجد نشاطات بعد.</td></tr>';
      return;
    }
    const ACTION_NAMES = {
      login: 'تسجيل دخول', logout: 'تسجيل خروج',
      create_student: 'إضافة طالب', update_student: 'تعديل طالب', delete_student: 'حذف طالب',
      upload_attachment: 'رفع ملف', delete_attachment: 'حذف ملف',
      delivery_change: 'تسليم مستند', bulk_import: 'استيراد من إكسيل',
      create_employee: 'إضافة موظف', update_employee: 'تعديل موظف', delete_employee: 'حذف موظف',
      activate_employee: 'تفعيل موظف', deactivate_employee: 'إيقاف موظف'
    };
    let html = '';
    log.forEach(e => {
      ...
      const details = e.details || (e.targetName ? e.targetName : '');
      html += '<tr>' +
        '<td style="font-size:0.72rem;color:var(--text-muted);white-space:nowrap;">' + dateStr + '</td>' +
        '<td style="font-weight:600;">' + escapeHtml(e.employeeName) + '</td>' +
        '<td><span class="log-action ' + escapeHtml(e.action) + '">' + escapeHtml(actionName) + '</span></td>' +
        '<td style="font-size:0.78rem;color:var(--text-secondary);">' + escapeHtml(details) + '</td>' +
      '</tr>';
    });
    tb.innerHTML = html;
  } catch (e) { console.error('renderActivityLog error:', e); }
};

window.clearActivityLog = async function() {
  if (!isOfflineMode && db) {
    try { await db.ref('activityLog').remove(); } catch (e) {}
  }
  localStorage.removeItem('lotus_activity_log');
  renderActivityLog();
  showToast('تم مسح السجل', 'success');
};
